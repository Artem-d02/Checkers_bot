...
namespace gotg
{
	...
	bool Graph_of_the_game::is_cage_empty(const size_t x, const size_t y) const throw(std::exception&)
	{
		...
		{
			throw std::out_of_range("Error: out of range in field_of_game in function \"Graph_of_the_game::is_cage_empty\"");	//	излишне
		}
	...
	std::vector<turn::Turn> Graph_of_the_game::get_all_turns_for_checker(turn::Cage_on_field& cage, VALUES_FOR_FIELD checker_in_this_cage, bool is_it_first_call) const noexcept
	{
		...
		static int direction_of_turns = 0;	// далее может быть только 1 или -1
		...
		//	не может быть 2 шагов по одинаковым клеткам
		try
		{
			turn::Cage_on_field new_cage(cage.x - 2, cage.y + 2);
		...
		try
		{
			turn::Cage_on_field new_cage(cage.x + 1, cage.y + direction_of_turns);		//	шашки игрока располгагаются вверху поля и двигаются вниз
	...
	{
		//	в предположении что turn задан корректно
		...
		game_field[(*--turn_to_it_field.get_full_turn().end()).get_to().y][(*--turn_to_it_field.get_full_turn().end()).get_to().x] = inverse_value_for_field(who_is_turn_now);		//	исправить!
	}

	void make_full_graph(std::shared_ptr<Graph_of_the_game> graph_ptr, const size_t deep_of_prediction, const std::shared_ptr<std::vector<bt::Info_about_one_first_turn>> info_about_eaten_checkers_vector_ptr, std::function<double(const bt::Eaten_checkers&)> analyse_value_finder)
	{
		...
		for (auto& subgraph : graph_ptr->subgraphs)
		{
			...
			make_full_graph(subgraph, deep_of_prediction, info_about_eaten_checkers_vector_ptr, analyse_value_finder);
			if (deep == 0)
			{
