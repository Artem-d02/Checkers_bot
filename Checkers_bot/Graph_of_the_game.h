...
namespace gotg
{
	...
	private:
		...
		size_t count_of_my_checkers_eaten;		// количество шашек, которое съел противник
		size_t count_of_enemy_checkers_eaten;	// количество шашек, которое съел игрок
	...
	public:
		...
		friend void make_full_graph(std::shared_ptr<Graph_of_the_game>, const size_t, const std::shared_ptr<std::vector<bt::Info_about_one_first_turn>>, std::function<double(const bt::Eaten_checkers&)>);
	};

	extern void make_full_graph(std::shared_ptr<Graph_of_the_game>, const size_t, const std::shared_ptr<std::vector<bt::Info_about_one_first_turn>>, std::function<double(const bt::Eaten_checkers&)>);
